...
public class BookingService {
    ...
    public Booking create(Booking booking) {
        ...
        if (booking.getUserId().equalsIgnoreCase(availability.getResourceName())) {
        ...
        if (activeBookings >= availability.getMaxSlots()) {
            ...
            List<Availability> alternatives = availabilityRepository.findByResourceNameContainingIgnoreCaseAndIsActiveTrue(availability.getResourceName())
                    .stream()
    ...
    public Booking cancel(Long id) {
        return bookingRepository.findById(id).map(booking -> {
            ...
            notificationService.markRelatedNotificationsAsRead(saved.getId());
            notificationService.notifyClientOfBookingUpdate(saved, tn.esprit.reservation.entities.Notification.NotificationType.BOOKING_CANCELLED);
            return saved;
    ...
    public Booking confirm(Long id) {
        return bookingRepository.findById(id).map(booking -> {
            ...
            Booking saved = bookingRepository.save(booking);
            notificationService.markRelatedNotificationsAsRead(saved.getId());
            notificationService.notifyClientOfBookingUpdate(saved, tn.esprit.reservation.entities.Notification.NotificationType.BOOKING_CONFIRMED);
            return saved;
        }).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Booking not found with id: " + id));
    }
