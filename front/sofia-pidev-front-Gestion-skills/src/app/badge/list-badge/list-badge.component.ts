...
export class ListBadgeComponent implements OnInit {
  ...
  assignBadge(badge: Badge): void {
    if (!badge.id) return;
    this.isLoading = true;
    this.badgeService.assignById(badge.id).subscribe({
      next: (response) => {
        this.isLoading = false;
        this.feedbackType = 'success';
        const count = response?.assignedRewards ?? 0;
        this.feedbackMessage = `Assignment complete. ${count} user(s) received the "${badge.name}" badge.`;
        this.refresh();
      },
      error: (error) => {
        console.error(error);
        this.isLoading = false;
        this.feedbackType = 'error';
        this.feedbackMessage = 'Assignment action failed.';
      }
    });
  }
