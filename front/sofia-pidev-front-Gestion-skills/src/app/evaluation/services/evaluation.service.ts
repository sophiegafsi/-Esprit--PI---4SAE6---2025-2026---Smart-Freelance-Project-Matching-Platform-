...
export class EvaluationService {
  private readonly fallbackEvaluationUrl = 'http://localhost:8081';
  ...
  getEligibleTargets(email: string): Observable<{ name: string, email: string }[]> {
    return this.http.get<{ name: string, email: string }[]>(`${this.evaluationBaseUrl()}/evaluations/eligible-targets/${encodeURIComponent(email)}`);
  }
