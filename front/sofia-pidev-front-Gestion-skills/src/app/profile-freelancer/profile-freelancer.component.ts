...
export class ProfileFreelancerComponent implements OnInit {
  ...
  freelancerView = {
    name: '',
    title: '',
    image: 'https://ui-avatars.com/api/?name=Freelancer&size=200&background=6366f1&color=fff&rounded=true',
    about: '',
    rate: 0,
    rateUnit: '/h',
    rating: 5,
  ...
  projects: Project[] = [
    {
      ...
      image: 'https://ui-avatars.com/api/?name=Project&size=200&background=8b5cf6&color=fff'
    }
  ];
  ...
  updateView(user: any): void {
    ...
    const initials = encodeURIComponent(((user.firstName || '') + ' ' + (user.lastName || '')).trim() || 'User');
    this.freelancerView.image = user.imageUrl || `https://ui-avatars.com/api/?name=${initials}&size=200&background=6366f1&color=fff&rounded=true`;
