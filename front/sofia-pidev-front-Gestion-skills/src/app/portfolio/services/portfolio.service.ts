...
export class PortfolioService {
  private readonly baseUrl = 'http://localhost:8081/portfolio';
  ...
  getSkillCredibility(freelancerId?: string, limit = 10): Observable<SkillCredibility[]> {
  ...
  getSkillRanking(freelancerId?: string, limit = 10): Observable<SkillRanking[]> {
    const params: Record<string, string> = { limit: String(limit) };
    if (freelancerId && freelancerId !== '0') params['freelancerId'] = String(freelancerId);
    return this.http
  ...
  getProfileStrength(freelancerId?: string): Observable<ProfileStrength> {
  ...
  getProfileStatistics(freelancerId?: string): Observable<ProfileStatistics> {
    const params: Record<string, string> = {};
    if (freelancerId && freelancerId !== '0') params['freelancerId'] = String(freelancerId);
    return this.http
  ...
  downloadAnalyticsPdf(freelancerId?: string): Observable<Blob> {
    const params: Record<string, string> = {};
    if (freelancerId && freelancerId !== '0') params['freelancerId'] = String(freelancerId);
  ...
  private normalizeAchievement(raw: Record<string, unknown>): Achievement {
    return {
      ...
      freelancerId: String(raw['freelancerId'] ?? raw['freelancer_id'] ?? ''),
    };
  }
  ...
  private normalizeProfileStrength(raw: Record<string, unknown>): ProfileStrength {
    return {
      freelancerId: String(raw['freelancerId'] ?? ''),
