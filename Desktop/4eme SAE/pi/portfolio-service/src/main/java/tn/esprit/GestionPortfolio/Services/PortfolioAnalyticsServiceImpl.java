...
public class PortfolioAnalyticsServiceImpl implements IPortfolioAnalyticsService {
    ...
    @Override
    public List<SkillCredibilityDto> getSkillCredibility(String freelancerId, Integer limit) {
        String id = resolveId(freelancerId);
        AnalyticsSnapshot snapshot = loaderService.loadSnapshot(id);
        return limitRows(skillAnalyticsService.buildCredibilityRows(snapshot), limit);
    }

    @Override
    public List<SkillRankingDto> getSkillRanking(String freelancerId, Integer limit) {
        String id = resolveId(freelancerId);
        AnalyticsSnapshot snapshot = loaderService.loadSnapshot(id);
    ...
    @Override
    public ProfileStrengthDto getProfileStrength(String freelancerId) {
        String id = resolveId(freelancerId);
        AnalyticsSnapshot snapshot = loaderService.loadSnapshot(id);
        return profileAnalyticsService.buildProfileStrength(id, snapshot);
    }

    @Override
    public ProfileStatisticsDto getProfileStatistics(String freelancerId) {
        String id = resolveId(freelancerId);
        AnalyticsSnapshot snapshot = loaderService.loadSnapshot(id);
        List<SkillCredibilityDto> credibilityRows = skillAnalyticsService.buildCredibilityRows(snapshot);
        List<SkillRankingDto> rankingRows = skillAnalyticsService.buildRankingRows(credibilityRows);
        return profileAnalyticsService.buildProfileStatistics(id, snapshot, credibilityRows, rankingRows);
    }

    private String resolveId(String freelancerId) {
        if (freelancerId != null && !freelancerId.trim().isEmpty() && !freelancerId.equals("0")) {
            return freelancerId;
        }
