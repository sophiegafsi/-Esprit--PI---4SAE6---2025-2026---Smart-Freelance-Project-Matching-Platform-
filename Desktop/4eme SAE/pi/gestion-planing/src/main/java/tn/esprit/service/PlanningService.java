...
public class PlanningService {
    ...
    public List<Planning> getAllPlannings() {
        return planningRepository.findByUserId(getCurrentUserId());
    }
    ...
    public Planning addPlanning(Planning planning) {
        ...
        planning.setUserId(getCurrentUserId());
        return planningRepository.save(planning);
    }

    public Planning updatePlanning(Long id, Planning planningDetails) {
        Planning planning = getPlanningById(id); // already verifies ownership
    ...
    public List<Planning> searchPlannings(String keyword) {
        ...
        List<Planning> titleDescMatches =
                planningRepository.findByUserIdAndTitleContainingIgnoreCaseOrUserIdAndDescriptionContainingIgnoreCase(
                        userId, lowerKeyword,
                        userId, lowerKeyword
                );

        List<Planning> all = planningRepository.findByUserId(userId);
    ...
    public PlanningProgressResponse getPlanningProgress(Long planningId) {
        ...
        if (totalTasks == 0) {
            return new PlanningProgressResponse(planningId, 0, 0, 0, 0, 0.0);
        }
        ...
        return new PlanningProgressResponse(planningId, totalTasks, doneTasks, inProgressTasks, todoTasks, progress);
    }
    ...
    public PlanningWeightedProgressResponse getPlanningWeightedProgress(Long planningId) {
        ...
        for (Task task : planning.getTasks()) {
            ...
            if (task.getPriority() == TaskPriority.MEDIUM) priorityWeight = 2;
            else if (task.getPriority() == TaskPriority.HIGH) priorityWeight = 3;

            int baseProgress = 0;
            if (task.getStatus() == TaskStatus.IN_PROGRESS) baseProgress = 50;
            else if (task.getStatus() == TaskStatus.DONE) baseProgress = 100;
        ...
        double weightedProgress = totalWeight > 0 ? weightedScoreSum / totalWeight : 0.0;
        weightedProgress = Math.round(weightedProgress * 10.0) / 10.0;

        return new PlanningWeightedProgressResponse(planningId, planning.getTasks().size(), weightedProgress);
    }

    public List<PlanningDailyLoadResponse> getPlanningDailyLoad(Long planningId) {
        Planning planning = getPlanningById(planningId);

        if (planning.getTasks() == null || planning.getTasks().isEmpty()) return new ArrayList<>();

        Map<LocalDate, Long> minutesByDate = new LinkedHashMap<>();
        Map<LocalDate, Integer> taskCountByDate = new LinkedHashMap<>();

        for (Task task : planning.getTasks()) {
            if (task.getTaskDate() == null || task.getStartTime() == null || task.getEndTime() == null) continue;
        ...
        for (LocalDate date : minutesByDate.keySet()) {
            ...
            result.add(new PlanningDailyLoadResponse(date, taskCount, totalMinutes, totalHours));
        }
        return result;
    }
    ...
    public PlanningEfficiencyResponse getPlanningEfficiency(Long planningId) {
        ...
        if (planning.getTasks() == null || planning.getTasks().isEmpty()) {
            return new PlanningEfficiencyResponse(planningId, 0, 0, 0.0, 0.0, 0.0, "LOW");
        }

        List<Task> validTasks = planning.getTasks().stream()
                .filter(task -> task.getTaskDate() != null && task.getStartTime() != null && task.getEndTime() != null)
                .toList();
        ...
        for (LocalDate date : tasksByDate.keySet()) {
            ...
            for (int i = 0; i < dayTasks.size() - 1; i++) {
                ...
                if (gap > 0) wastedMinutes += gap;
            }
        }

        double averageTaskDuration = Math.round((totalTaskMinutes / (double) validTasks.size()) * 10.0) / 10.0;
        double taskDensity = (totalTaskMinutes + wastedMinutes) > 0
                ? totalTaskMinutes / (double) (totalTaskMinutes + wastedMinutes)
                : 0.0;
        ...
        if (averageTaskDuration < 30) durationPenalty = 15;
        else if (averageTaskDuration > 180) durationPenalty = 10;

        double efficiencyScore = Math.min(100, Math.max(0,
                Math.round((100 - wastedPenalty - durationPenalty + taskDensity * 30.0) * 10.0) / 10.0));

        String efficiencyLevel;
        if (efficiencyScore >= 75) efficiencyLevel = "HIGH";
        else if (efficiencyScore >= 50) efficiencyLevel = "GOOD";
        else if (efficiencyScore >= 30) efficiencyLevel = "MEDIUM";
        else efficiencyLevel = "LOW";

        return new PlanningEfficiencyResponse(planningId, validTasks.size(), wastedMinutes,
                averageTaskDuration, taskDensity, efficiencyScore, efficiencyLevel);
    }
}