...
public class EvaluationController {
    ...
    @GetMapping("/eligible-targets/{email}")
    public ResponseEntity<?> getEligibleTargets(@PathVariable String email) {
        try {
            return ResponseEntity.ok(evaluationService.getEligibleEvaluationTargets(email));
        } catch(Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(500).body("Error: " + e.getMessage() + " | StackTrace: " + java.util.Arrays.toString(e.getStackTrace()));
        }
    }
}